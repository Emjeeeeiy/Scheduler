...
- The Auth Shell's line-based frame — a topbar rule, a column divider, corner-tick marks — is now the app's own base unit everywhere, not a screen-specific exception: `.frame`, `FrameTicks`, `.stack`, and `.divided-row` carry it into Dashboard, Day, Week, Month, and every list manager. A modal is the sole holdout, since content floating above the page still needs a boxed shape and a shadow to read as its own layer — see Components
...
- **Container:** a *container* — the sidebar, a `.frame`, a stat tile, the mobile bottom bar — carries no fill of its own in either theme; it sits flat on Paper. This used to be a themed token (`--surface-card`, split because light and dark wanted different answers); it went away along with card shadows, once "flat on the page" stopped being dark mode's special case and became the rule everywhere — and the hairline border that used to do the separating for a card at rest is gone too, replaced by a frame's corner ticks and a section's hairline divider (see Cards & Frames).

  This is exactly why floating content needs its own explicit surface. `.modal__panel` carries its own border, radius, and padding directly (see Cards & Frames) and takes the Panel surface explicitly — a page-coloured modal in dark would be the same near-black as the scrim behind it, and unlike a container, a modal has to read as raised.
...
- **Item** (`--text-item` / `--font-weight-item`, 550, 14px, **Inter**): The **name of a thing** — a task, event, notification, or index row — plus nav links. One weight for this role wherever it appears: the block, the chip, the row, the notification, the day-peek entry, and the item index all set it identically, rather than each picking its own.
- **Label** (`--text-label` / `--font-weight-label`, 500, 13px, **Inter**): What a field or a figure is *called* — field labels, stat-tile and hero labels, small ghost/danger buttons, filter chips.
...
**The Name-Versus-Detail Rule.** Below 15px the steps are 1px apart and carry no hierarchy on their own — 14/13/12/11px all read as "small". **Weight and colour do the separating, size only follows.** A thing's name takes `--font-weight-item` (550) in primary ink; the detail about it takes `--font-weight-body` (400) in muted ink. A new component showing "a title and its timestamp" reuses that pair rather than inventing a third size. This is the rule the system had in prose and not in code: row titles shipped at 400, indistinguishable from their own metadata except by colour.
...
A fixed 232px sidebar (sticky, its own scroll) plus a centered content column capped at 1320px, padded 20px/24px/64px (top/sides/bottom). The base unit is the `.frame`: no border, no radius, an unfilled 18px-padded container marked only by its corner ticks — nearly every screen is one frame, its content divided into sections by a hairline (`.stack`, `.divided-row`) rather than by boxing each one separately.
...
- **640px** — tighter page padding, the hero number steps down to 42px, paired form fields stack to one column, frames lose 4px of padding.

Grids follow content, not a fixed column count: the stat-tile row, the chart row, and the queue row (`.queue-row`, Next up beside Overdue) all use `repeat(auto-fit, minmax(…, 1fr))` so they reflow rather than break, and all three carry `.divided-row` for the hairline between columns instead of a gap. `auto-fit` is doing real work in the queue row: Overdue only renders when something is overdue, and Next up takes the whole width on its own with no second rule — and because `.divided-row` only draws a rule between *existing* children, Next up alone gets no stray divider either. `.divided-row`'s `align-items: stretch` carries the divider the full height of the row even when Overdue is shorter than Next up, since there is no card box left to pad out to match — a continuous line reads as one row, a line that stops short reads as a mistake. The time grid is the one true fixed grid — a 60px gutter plus 7 equal day columns (1 on the Day view), scaled by a single `--hour-height` variable.

## Elevation & Depth

Flat, full stop: a section of the app carries no fill, no border, and no shadow of its own, ever — only a `.frame`'s corner ticks and a `.stack`/`.divided-row`'s hairline divider separate one region from the next, on the same page color everything sits on. Depth exists in exactly one place: content that genuinely floats above the page, which is also the one place a boxed, bordered panel survives (the modal — see `.modal__panel` under Components). Hover and active feedback on a control is a background-color step (Panel → Hover Surface); nothing lifts, scales, moves, or gains a border on interaction.
...
- **Floating** (`0 6px 24px rgba(11,11,11,0.08)`): The one shadow tier actually in use, reserved for content that genuinely floats above the page's own stacking order: the modal panel, the notification and account dropdowns, the day-peek popover, the chart tooltip. Nothing else gets a shadow — not a `.frame`, not a stat tile, not the sidebar.

### Named Rules
**The Flat-By-Default Rule.** Surfaces are flat, always. A frame's corner ticks and a hairline divider do all the work of separating one region from another; there is no second, raised surface color to step up to, and no border drawn around a resting section any more. A shadow — and the one surviving boxed shape, the modal panel — is reserved entirely for content that floats above the page's own stacking order; nothing else lifts, scales, or moves on interaction.

## Shapes

Tightened from the system's original 6/10/16px scale to read sharper and more consistent: a core three-step scale — 4px / 7px / 10px — covers every control and the modal panel (the one section that still has a radius), plus two smaller steps that shrink in step with the element: 3px on chip-like marks (blocks, chips, the month-cell add button) and 2px on the smallest marks of all — a 9–10px legend swatch, a tag-bars dot, or the drag-hover time tooltip — where anything larger would read as a circle rather than a softened square. A full 999px pill closes the scale for anything circular or fully rounded (badges, the avatar, filter chips) — pills are a distinct shape choice, not a "roundness" the tightened scale reaches into. The rule underneath all of it: radius shrinks with the element, never stays fixed regardless of size. Every corner is soft and uniform; nothing is cut, asymmetric, or sharp. Borders are hairline (1px, 10% alpha) throughout — the system never reaches for a heavier stroke to add emphasis.

One exception remains: a `.frame` — the base unit everywhere except a modal — has no rounded corners to speak of at all, unboxed and separated from its neighbours by a single square hairline rather than a radius. The modal panel is where the tightened radius scale still applies in full — see Cards & Frames under Components.
...
The bordered, radiused, padded `.card` box — once the base unit of nearly every screen — is gone everywhere except a modal. What replaced it, app-wide, is the structural language the Auth Shell introduced when it was still a one-screen exception:

- **`.frame`:** the base unit for a page-level section now — Dashboard, Day, Week, Month, the mini calendar, the setup screen. No border, no radius, no fill, no shadow: `position: relative` and 18px padding (14px at the 640px breakpoint), nothing else. A frame's own edge is read entirely from its content's inset and its corner ticks, never from a drawn line around it.
- **`FrameTicks`:** the four corner-tick marks (two hairline strokes each, in Baseline) that read as a frame's boundary in place of a border — render as the frame's first child. The one borrowed, purely decorative motif in the system, now shared everywhere a `.frame` appears rather than confined to the sign-in screen.
- **`.stack`:** a vertical sequence of sections that separate with a hairline (`border-top`) instead of a gap — the device behind the Dashboard's row-groups, generalised from the Auth Hero's own `.auth-hero__row`. A row-group that is *also* a `.divided-row` carries **no padding of its own** here — padding on the row would inset its content box from its own border, and `.divided-row`'s `align-items: stretch` can only stretch a column divider to fill that smaller box, stopping it short of the hairline instead of touching it. A plain (non-divided) row-group takes the usual 18px padding directly, since it has no column divider that needs to reach anywhere.
- **`.divided-row`:** a horizontal row whose children separate with a hairline (`border-left`) instead of a gap — the device behind the stat-tile row, the Next up/Overdue queue, and the chart row, generalised from the Auth Hero's own `.auth-hero__stats`. Because the rule only falls between *existing* children, a row that loses an item (Overdue, when nothing is overdue) loses its divider too, with no extra logic. The 18px breathing room lives on each *child* (all four sides, not just left/right) rather than on the row itself — the other half of the `.stack` trade above: with the row-group's own box unpadded, a column divider's `align-items: stretch` reaches the row's full height and visibly touches the hairline above and below it, a connected grid rather than two rulers that happen to sit near each other.
- **Hero section:** the Dashboard's "Planned today" section additionally carries the Glow Neutral radial-gradient wash (positioned top-right, fading by 70%) — the one place a background is anything but flat. It is a **column anchored at both ends**, not a stack: `.hero__head` (label + figure) at the top, and a bottom cluster — the full-width load bar, its reference note, a hairline rule, then the day's breakdown paired with its action — carried down by `margin-top: auto` on `.hero__load`. The auto margin sits on the load bar rather than the footer on purpose: it moves the whole cluster as one, leaving a single deliberate gap under the figure instead of stranding furniture on both sides of a void.
- **Stat tile:** no box of its own any more — a column of content (a muted 15px category icon top-right of its label, the figure, an optional hint) living inside a `.divided-row` cell, separated from its neighbours by the row's own hairline. `--color-critical`/`--color-good` tones recolor the icon along with the value.
- **Modal panel (the one surviving box):** `.modal__panel` carries its own border, `{rounded.lg}` radius, and 18px padding (14px at 640px) directly — no longer borrowed from `.card`, since nothing else needs that shape any more — plus the raised Panel surface and Floating shadow, because a modal has to read as a distinct layer above a dimmed page. Every other section in the app reads its structure from lines alone.
...
- **Tag select:** `TagSelect` — the Task/Event editor's plain `<select>` for assigning a tag, shared by both forms. A native `<option>` can't reliably carry a background color across browsers, so the picker itself stays plain and an 8px dot sits 10px in from the left edge instead, tracking the *selected* tag's color (a hollow Baseline-bordered ring for "No tag") — the one part of the control a page can actually paint, and enough to keep a tag looking like a tag everywhere it's chosen, not just everywhere it's already assigned.
...
- **Section rules:** the hairlines above Tags/All items and above the footer icons (`.sidebar__tools`, `.sidebar__footer`) run the sidebar's full width, not just its padded content width — a negative margin matching the sidebar's own 14px inset pushes each divider out to the sidebar's real edges, so it visibly touches `.sidebar`'s own border-right on the way past rather than stopping a padding short of it. The same connected-line trade as `.stack`/`.divided-row` and the Auth Hero's own row/stat dividers, applied to the one other place in the chrome a hairline meets a hairline.
...
### Auth Shell (origin of the frame system)

Sign-in and sign-up (`.auth-shell`) are built entirely from **line, not fill**: a topbar rule, one hairline column divider, and a corner-tick frame — no element on this screen ever carries a background color of its own, so nothing reads as a box. It used to be a fixed, always-dark surface with its own literal violet-to-black gradient; it now follows the same light/dark tokens, and the same zero-accent rule, as everywhere else — the ink-invert button, the ink focus ring, and the underlined link it uses are the app's ordinary components now, not a scoped override. This screen is no longer a structural exception: `.frame`, `FrameTicks`, `.stack`, and `.divided-row` (see Cards & Frames under Components) carried its line-based language to the rest of the app, and what's documented here is now the shared vocabulary, not a one-screen departure from it.

- **Auth Topbar** (`.auth-topbar`, full width): the brand mark and "Cadence" on the left, a `.icon-button` theme toggle on the right (identical cycle behavior to the app shell's) — this screen precedes the toggle everywhere else and still needs one, always reachable regardless of viewport. A single `1px --color-border` rule underneath separates it from the content frame; there is no fill behind it, just `--color-page`.
- **Auth Body** (`.auth-body`, everything under the topbar): the bounded content frame, 28px inset (20px ≤1080px, edge-to-edge ≤860px). `<FrameTicks />` (`.frame-tick`, two 1px hairline strokes in `--color-baseline`) sits inset 12px at each of its four corners — this screen's own frame, hidden ≤860px where it goes edge-to-edge, but the same corner-tick component every other `.frame` in the app now renders too.
- **Auth Hero** (left column inside the body, roughly 60–75% on a desktop viewport, hidden below 860px): plain centered copy — a headline pitch in `--color-text-primary`, a lead in `--color-text-secondary` — with no background and no border of its own. A single `border-right: 1px solid var(--color-border)` is the only thing separating it from the form; that one hairline is the entire "divider," not a gap between two boxes. Its own `.auth-hero__row` dividers carry the same connected-line fix `.stack` was later generalised from: the stats row (`.auth-hero__row--last`) takes zero top padding of its own, and each `.auth-hero__stat` takes it instead, so a stat's column divider stretches all the way to the preview row's hairline above it and touches it — the model this pattern was copied from had briefly drifted out of sync with the copy.
- **Auth Panel** (right column inside the body: `clamp(480px, 50%, 700px)`, sized to its 380px content plus a gutter rather than growing with the viewport; falls back to filling the frame's full width below 860px): a centered 380px-max form column with no panel background or border either — a white Google button (real multi-color "G," the one exception to the icon grammar, still literal-colored and bordered in `--color-border` so it stays legible on a light-mode page), a divider, and the app's own themed `.field`/`.input`/`.auth-form__switch`, `.primary-button`, and `.link-button` — no scoped overrides left, since ink-invert and the ink focus ring are already the app-wide default. Every visible edge here belongs to an individual control (an input's own hairline, the Google button's own hairline), never to a wrapping panel.
- **Why line instead of card:** the previous fixed palette matched a supplied reference's violet-to-black gradient exactly; a later reference pinned a monochrome dev-tool aesthetic built from rules, a corner-tick frame, and un-filled bordered controls rather than filled panels — confirmed as "no cards, just lines and +." That verdict has since generalized to the whole app: the topbar rule stays this screen's own (the rest of the app has the sidebar instead), but the corner-tick frame and the column-divider idiom are `FrameTicks`, `.stack`, and `.divided-row` now, carried everywhere a `.card` used to be.
...
The calendar's own vocabulary for a scheduled task, and the one place a colored border appears anywhere in the system. A `.block` gets a 3px left border in the task's own tag color, a matching 15%-tint background (`color-mix(in srgb, var(--tag) 15%, var(--color-surface-1))`), and 3px corners. This is functional color-coding for the task's category, not a decorative accent — it is the one sanctioned exception to "no colored left borders," confirmed for this component alone and not a pattern to extend to a generic frame or list item. A repeating occurrence adds a small repeat icon after its time label; a completed block drops its border and gets a strikethrough title.
...
- **Do** give hover/active feedback with a background-color step (Panel → Hover Surface → Sunken Surface) on any control that has a fill to begin with — never a shadow, lift, scale transform, or a border a resting `.frame` doesn't otherwise carry.
...
- **Do** resolve a tag's color through the `--tag` custom property and a themed `var(--color-tag-*)` token, never an inline hex — the only way a component works correctly in both themes.
...
- **Do** build a new page-level section from `.frame` + `<FrameTicks />`, with `.stack` for the sections stacked inside it and `.divided-row` for anything side by side — never a bordered box. The modal panel is the only place `.card`'s old border-radius-padding shape survives; everything else reads its structure from a corner-tick frame and a hairline divider.
...
- **Don't** give a `.frame` a border, radius, or fill of its own to make it read as "boxed" again — corner ticks and a hairline divider are the entire vocabulary now, the retired `.card` look does not get reintroduced piecemeal. The modal panel is the one deliberate, named exception, because floating content still needs a shape and a shadow to read as a separate layer; it is not a precedent for anything at rest.
- **Don't** add a colored left border, top border, or accent stripe to a generic frame, list item, or callout. The Time-Grid Block's left border is functional tag-color coding on the app's signature calendar element — it does not generalize to anything else.
- **Don't** reach for a shadow as hover feedback or as decoration on anything at rest. Shadow is reserved for content that floats above the page's own layer (modal, dropdown).
