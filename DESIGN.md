...
- **Container** (`--surface-card`: Panel in light / **Paper in dark**): The surface a *container* sits on — the sidebar, a card, a stat tile, the mobile bottom bar. Split out from Panel because the two themes want different answers. Light keeps its step above Paper. **Dark sits containers flat on the page** and lets the hairline border do the separating, which is the direction dark was already heading: its card shadow is an edge ring rather than a blur, because a black shadow is invisible against a near-black page. Declared as `var(--surface-1)` / `var(--page)` rather than repeated literals, so it cannot drift from the values it is meant to equal.

  The split is what keeps floating surfaces floating. `.modal__panel` carries `.card` for its shape and padding but takes the raised surface back explicitly — a container-coloured modal in dark would be the same near-black as the scrim behind it.
