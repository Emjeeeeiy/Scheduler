 ...
 * own once it stops being true is the point. A scheduled push run every
 * ~5 minutes needs the opposite: state. Without it, a task sitting inside
 * the "starting soon" window would fire a fresh push on every single poll
 * — the same task, over and over, for the whole hour before it starts.
 *
 * Kept exactly as small and pure as buildNotifications itself: given the
 * current list and what was already sent, decide what's new — and let go of
 * anything that resolved (was done, rescheduled, or aged out of its window),
 * so a task can notify again later if it genuinely comes back around
 * (rescheduled, then overdue a second time). No Firebase import — exercised
 * directly by functions/test/pushSelection.test.js.
 */

/**
 * @param items         buildNotifications' current output for one user
 * @param alreadySentIds ids pushed on a previous run (plain array, as stored
 *                        on the user's pushState doc)
