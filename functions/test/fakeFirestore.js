...
/** Writes `users/{uid}/tasks/{id}` for each task in `tasks` (each already
     shaped `{ id, ...fields }`), `users/{uid}/fcmTokens/{token}` for each
     token, and any extra top-level fields (like `dailyDigestEnabled`, `email`,
     `timeZone`) onto `users/{uid}` itself. */
export function seedUser(store, uid, { profile = {}, tasks = [], fcmTokens = [] } = {}) {
  store.docs.set(`users/${uid}`, profile)
  for (const { id, ...fields } of tasks) store.docs.set(`users/${uid}/tasks/${id}`, fields)
  for (const token of fcmTokens) store.docs.set(`users/${uid}/fcmTokens/${token}`, { token })
}
